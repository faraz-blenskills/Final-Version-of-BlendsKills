...
export function SiteNav() {
  ...
  return (
    <>
      {/* Scrolls away with the page instead of staying pinned like the tab bar below. */}
      <Link
        to="/"
        aria-label="BlendSkills Home"
        className="glass-pill absolute left-5 top-6 z-40 flex items-center rounded-full p-3 sm:left-8"
      >
        <img src={logoMark} alt="BlendSkills" className="h-11 w-auto brightness-0" />
      </Link>

      <header className="pointer-events-none fixed inset-x-0 top-0 z-50">
        <div className="mx-auto flex max-w-[1400px] items-center justify-end px-5 pt-6 sm:px-8 md:justify-center">
